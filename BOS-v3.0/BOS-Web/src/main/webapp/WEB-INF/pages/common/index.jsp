...
/* 	$.post("${pageContext.request.contextPath}/json/menu.json",function(data){ */
	$.post("${pageContext.request.contextPath}/function_ajaxTreeMenu",function(data){
		...
		window.setTimeout(function(){
			$.messager.show({
				title:"消息提示",
				msg:'欢迎登录，<shiro:user><shiro:principal property="username"/> </shiro:user>！ <a href="javascript:void" onclick="top.showAbout();">联系管理员</a>',
				timeout:5000
			});
		},3000);
		/*************/
		...
		$("#btnEp").click(function(){
			/*
			* 1，获取两个输入框，校验输入框输入密码是否一致，不一致弹框提示，
			  2，当输入框输入值一致时候弹框，确认是否修改
			  3，当确认修改，发送ajax请求，把新密码发送过去
			*/
			var txtNewPassVal = $("#txtNewPass").val();
			var txtRePassVal = $("#txtRePass").val();
			if($.trim(txtNewPassVal)==""){
				$.messager.alert('警告','新密码不能为空','warning');return;
			}
			var reg =/\s+/;
			if(reg.test(txtNewPassVal)){
				$.messager.alert('警告','新密码不能包含空白字符','warning');return;
			}else
			if($.trim(txtNewPassVal) != $.trim(txtRePassVal)){
				$.messager.alert('警告','两次密码不一致','warning');return;
			}else {
				$.messager.confirm("提醒","请您确认修改？", function(){
					alert("修改密码");
					$.post("${pageContext.request.contextPath }/user_editpassword.action?", { password : txtRePassVal },function(data){
					//获取服务器返回数据，	
						if(data){
							$.messager.alert("信息", "修改成功", "info");
						}else{
							$.messager.alert("信息", "密码修改失败", "info");
						}
						// 窗口关闭 
						$("#editPwdWindow").window('close');
					},"json");
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				/* 直接跳转到validate.jsp,在该页面进行session销毁处理并跳转到login.jsp */
				location.href = '${pageContext.request.contextPath }/loginAction_logout';
			}
		});
	}
	// 修改密码
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("宅急送 v1.0","设计: 荣哥<br/> 管理员邮箱: shengrong_xie@163.com <br/> QQ: 578440089");
	}
</script>
</head>
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="sessionInfoDiv"
			...
			[<strong><shiro:user><shiro:principal property="username"/> </shiro:user></strong>]，欢迎你！您使用[<strong>${pageContext.request.remoteAddr }</strong>]IP登录！
		</div>
