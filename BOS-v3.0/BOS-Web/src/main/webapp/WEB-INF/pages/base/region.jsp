			...
			onComplete : function(response){
			 ...
			 onSelect: function (self, element) {  
                 ...
                 if (!re.test(this.filename())) {  
                     alert("Only xls or xlsx file can be uploaded");  
                 }  
                 else {  
                     this.submit();  
                 }  
...
function doDblClickRow(rowIndex, rowData){
		...
		$("#postcode").removeClass("easyui-validatebox");
		$('#addRegionWindow').window("open");
	}
$.extend($.fn.validatebox.defaults.rules, {
	...
	uniquePostID: {
		validator: function(value){
			...
			$.ajax({
				type: "POST",
				url: "${pageContext.request.contextPath }/region_ajaxPostId.action",
				data: {postcode:value},
	...
	uniqueId: {
		validator: function(value){
			...
			$.ajax({
				type: "POST",
				url: "${pageContext.request.contextPath }/region_ajaxId.action",
				data: {id:value},
				async: false,/* 必须要指定异步为false */
				success: function(data){
					flag = data;
	...
	message: "区域编号重复,这是行政区域唯一标识<格式:省xx市xx区xx,eq:AHHFGX(安徽合肥高新区)>"
		}
	});
</script>	
</head>
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form id="_form" action="${pageContext.request.contextPath }/region_saveOrUpdate.action" method="post">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>邮编</td>
						<td><input type="text" id="postcode" name="postcode" class="easyui-validatebox" data-options="required:true,validType:['uniquePostID']"/></td>
					</tr>
					<!-- <tr>
						<td>简码</td>
