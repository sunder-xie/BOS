	...
	function doImport(){
		...
		$('#button-import').upload({
			...
			onComplete : function(data){
				if(data==null){
					$.messager.alert('信息',"导入成功✔️",'info');
				}else{
				var info = eval('('+data+')');
				$.messager.alert('信息',info.msg,'error');
				}
	...
	$.extend($.fn.validatebox.defaults.rules, {
		...
		uniqueId: {
			validator: function(value){
				...
				$.ajax({
					type: "POST",
					url: "${pageContext.request.contextPath }/subarea_ajaxId.action",
					data: {id:value},
		...
		message: "管理分区编号重复,这是逻辑分区唯一标识<格式:参考手册>"
			}
		});
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="region.id"  id="regionId"
    							data-options="valueField:'id',textField:'info',url:'${pageContext.request.contextPath }/region_queryRegion.action?',mode:'remote'" />  
						</td>
					</tr>
					<tr>
						<td>关键字</td>
